...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
    ...
    echo "Run: source ./scripts/load-proxmox-audit-env.sh" >&2
    exit 1
fi

_PROXMOX_SECRET_DIRECTORY="${HOME}/.config/homelab/terraform"
_DTCODE_SECRET_FILE="${_PROXMOX_SECRET_DIRECTORY}/dtcode-audit-token.json"
_DOM_SECRET_FILE="${_PROXMOX_SECRET_DIRECTORY}/dom-audit-token.json"
...
export TF_VAR_dtcode_api_token="$(
    jq -r '."full-tokenid" + "=" + .value' "${_DTCODE_SECRET_FILE}"
)"

export TF_VAR_dom_api_token="$(
...
echo "Read-only Proxmox Terraform credentials loaded for dtcode and dom."
